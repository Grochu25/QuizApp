namespace QuizCreator.ViewModels
{
    ...
    class QuizEditListViewModel : BaseViewModelClasses.ViewModel
    {
        private Model _model = new Model();
        private ViewModelChanger _viewModelChanger;
        private ObservableCollection<Quiz>? _quizes = new ObservableCollection<Quiz>();

        public QuizEditListViewModel(ViewModelChanger viewModelChanger)
        {
        ...
        private ICommand? _takeQuizMode = null;
        public ICommand TakeQuizMode
        {
            get
            {
                if (_takeQuizMode == null)
                    _takeQuizMode = new RelayCommand(
                        arg => {
                            _viewModelChanger.CurrentViewModel = new QuizListViewModel(_viewModelChanger);
                        },
                        arg => true
                        );
                return _takeQuizMode;
            }
        }
    }
}
